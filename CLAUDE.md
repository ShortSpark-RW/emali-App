...
4. **Dependency Injection**: Koin is used. `sharedModule` in `KoinModule.kt` defines all common dependencies. Platform-specific modules are added via `expect/actual` `targetModule`.
...
Koin is used for DI. The `sharedModule` in `di/KoinModule.kt` defines all common dependencies. Platform-specific modules are expected via `expect/actual` pattern with `targetModule`. `initializeKoin()` starts Koin with both modules.
